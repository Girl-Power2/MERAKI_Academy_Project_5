...
notes.AddNote = async (req, res) => {
  ...
  const query = `INSERT INTO provider_notes (user_id,provider_id,note) VALUES ($1,$2,$3) RETURNING *;`;
...
notes.GetNotebyProviderId = async (req, res) => {
    ...
    const query = `SELECT * from provider_notes INNER JOIN providers ON provider_notes.provider_id=providers.provider_id INNER JOIN users ON provider_notes.user_id=users.user_id WHERE provider_id=$1 AND user_id=$2 RETURNING *;`;
    ...
    try {
      ...
      if(response.rowCount){
          res.status(201).json({
              status:true,
              message:`All notes for the user ${users.firstName}`,
              data:response.rows
          })
      }
      else{
        res.status(404).json({
            status:false,
            message:`No recorded notes for the user ${users.firstName} with the id ${users.user_id} `,
            error:error.message
        })
      }
    } catch (error) {
